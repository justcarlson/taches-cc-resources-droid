...
When Droid executes this, the full command with expanded `$API_KEY` appears in the conversation.
</the_problem>

<the_solution>
Use `~/.factory/scripts/secure-api.sh` - a wrapper that loads credentials internally.

<for_supported_services>
```bash
# ✅ GOOD - No credentials visible
~/.factory/scripts/secure-api.sh <service> <operation> [args]

# Examples:
~/.factory/scripts/secure-api.sh facebook list-campaigns
~/.factory/scripts/secure-api.sh ghl search-contact "email@example.com"
```
</for_supported_services>
...
1. **Add operations to the wrapper** (`~/.factory/scripts/secure-api.sh`):
...
3. **Add credential placeholders to `~/.factory/.env`** using profile naming:

```bash
# Check if entries already exist
grep -q "YOURSERVICE_MAIN_API_KEY=" ~/.factory/.env 2>/dev/null || \
  echo -e "\n# Your Service - Main profile\nYOURSERVICE_MAIN_API_KEY=\nYOURSERVICE_MAIN_ACCOUNT_ID=" >> ~/.factory/.env

echo "Added credential placeholders to ~/.factory/.env - user needs to fill them in"
```
...
1. **Check for saved profile:**
   ```bash
   ~/.factory/scripts/profile-state get yourservice
   ```

2. **If no profile saved, discover available profiles:**
   ```bash
   ~/.factory/scripts/list-profiles yourservice
   ```
...
5. **Save user's selection:**
   ```bash
   ~/.factory/scripts/profile-state set yourservice <selected_profile>
   ```
...
7. **Make API call with profile:**
   ```bash
   ~/.factory/scripts/secure-api.sh yourservice:<profile> list-items
   ```
...
```bash
~/.factory/scripts/secure-api.sh yourservice:<profile> <operation> [args]

# Examples:
~/.factory/scripts/secure-api.sh yourservice:main list-items
~/.factory/scripts/secure-api.sh yourservice:main get-item <ITEM_ID>
```
...
Usage:
```bash
echo '{"name":"value"}' | ~/.factory/scripts/secure-api.sh service create-item
```
</post_with_json_body>
...
<credential_storage>
**Location:** `~/.factory/.env` (global for all skills, accessible from any directory)
...
```bash
set -a
source ~/.factory/.env 2>/dev/null || { echo "Error: ~/.factory/.env not found" >&2; exit 1; }
set +a
```
</credential_storage>
...
3. **Auto-create credential placeholders** - add empty fields to `~/.factory/.env` immediately when creating the skill
4. **Keep credentials in `~/.factory/.env`** - one central location, works everywhere
...
grep -q "YOUR_API_KEY=" ~/.factory/.env && echo "API key configured" || echo "API key missing"
```
</testing>
