...
**Core principle:** Droid automates everything with CLI/API. Checkpoints are for verification and decisions, not manual work.

## Checkpoint Types

### 1. `checkpoint:human-verify` (Most Common)

**When:** Droid completed automated work, human confirms it works correctly.
...
<task type="checkpoint:human-verify" gate="blocking">
  <what-built>[What Droid automated and deployed/built]</what-built>
  <how-to-verify>
...
**Key elements:**
- `<what-built>`: What Droid automated (deployed, built, configured)
...
**When:** Action has NO CLI/API and requires human-only interaction, OR Droid hit an authentication gate during automation.

**Use ONLY for:**
- **Authentication gates** - Droid tried to use CLI/API but needs credentials to continue (this is NOT a failure)
...
<task type="checkpoint:human-action" gate="blocking">
  <action>[What human must do - Droid already did everything automatable]</action>
  <instructions>
    [What Droid already automated]
    [The ONE thing requiring human action]
  </instructions>
  <verification>[What Droid can check afterward]</verification>
  <resume-signal>[How to continue]</resume-signal>
</task>
```

**Key principle:** Droid automates EVERYTHING possible first, only asks human for the truly unavoidable manual step.
...
<!-- If vercel returns "Error: Not authenticated", Droid creates checkpoint on the fly -->
...
<!-- After authentication, Droid retries the deployment -->
...
**Key distinction:** Authentication gates are created dynamically when Droid encounters auth errors during automation. They're NOT pre-planned - Droid tries to automate first, only asks for credentials when blocked.

See references/cli-automation.md "Authentication Gates" section for more examples and full protocol.

## Execution Protocol

When Droid encounters `type="checkpoint:*"`:
...
**DON'T:**
- Ask human to do work Droid can automate (deploy, create resources, run builds)
- Assume knowledge: "Configure the usual settings" ❌
- Skip steps: "Set up database" ❌ (too vague)
- Mix multiple verifications in one checkpoint (split them)
- Make verification impossible (Droid can't check visual appearance without user confirmation)
...
- Things Droid can verify programmatically (tests pass, build succeeds)
- File operations (Droid can read files to verify)
...
Place checkpoints:
- **After automation completes** - not before Droid does the work
- **After UI buildout** - before declaring phase complete
- **Before dependent work** - decisions before implementation
- **At integration points** - after configuring external services

Bad placement:
- Before Droid automates (asking human to do automatable work) ❌
...
<!-- NO CHECKPOINT NEEDED - Droid automated everything and verified programmatically -->
```
...
**Why bad:** Vercel has a CLI. Droid should run `vercel --yes`.

### ✅ GOOD: Droid automates, human verifies
...
**Why bad:** Droid has Write tool. This should be `type="auto"`.

## Summary

Checkpoints formalize human-in-the-loop points. Use them when Droid cannot complete a task autonomously OR when human verification is required for correctness.

**The golden rule:** If Droid CAN automate it, Droid MUST automate it.

**Checkpoint priority:**
1. **checkpoint:human-verify** (90% of checkpoints) - Droid automated everything, human confirms visual/functional correctness
...
**See also:** references/cli-automation.md for exhaustive list of what Droid can automate.
