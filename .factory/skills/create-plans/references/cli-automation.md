...
**Core principle:** If it has a CLI or API, Droid does it. Never ask the human to perform manual steps that Droid can automate.

This reference documents what Droid CAN and SHOULD automate during plan execution.
...
**Critical distinction:** When Droid tries to use a CLI/API and gets an authentication error, this is NOT a failure - it's a gate that requires human input to unblock automation.

**Pattern: Droid encounters auth error → creates checkpoint → you authenticate → Droid continues**
...
<!-- After authentication, Droid retries automatically -->
...
<!-- After key provided, Droid writes to .env and continues -->
...
**When Droid encounters authentication error during execution:**
...
**Key difference:** Droid does EVERYTHING possible first (account creation, API requests), only asks human for the one thing with no automation path.

## Quick Reference: "Can Droid automate this?"

| Action | CLI/API? | Droid does it? |
|--------|----------|-----------------|
...
**Default answer: YES.** Unless explicitly in the "NO" category, Droid automates it.
...
**The rule:** If Droid CAN do it, Droid MUST do it.

Checkpoints are for:
- **Verification** - Confirming Droid's automated work looks/behaves correctly
...
**This keeps the agentic coding workflow intact - Droid does the work, you verify results.**
