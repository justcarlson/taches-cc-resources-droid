...
# If complex:
mkdir -p ~/.factory/skills/{skill-name}/workflows
mkdir -p ~/.factory/skills/{skill-name}/references
# If needed:
mkdir -p ~/.factory/skills/{skill-name}/templates  # for output structures
mkdir -p ~/.factory/skills/{skill-name}/scripts    # for reusable code
```
...
```bash
cat > ~/.factory/commands/{skill-name}.md << 'EOF'
---
