<overview>
Droid-executable plans have a specific format that enables Droid to implement without interpretation. This reference defines what makes a plan executable vs. vague.

**Key insight:** PLAN.md IS the executable prompt. It contains everything Droid needs to execute the phase, including objective, context references, tasks, verification, success criteria, and output specification.
</overview>

<core_principle>
A plan is Droid-executable when Droid can read the PLAN.md and immediately start implementing without asking clarifying questions.

If Droid has to guess, interpret, or make assumptions - the task is too vague.
</core_principle>
...
<task type="checkpoint:human-verify" gate="blocking">
  <what-built>[what Droid automated]</what-built>
...
<type name="auto">
**Default task type** - Droid executes autonomously.
...
Use for: Everything Droid can do independently (code, tests, builds, file operations).
</type>

<type name="checkpoint:human-action">
**RARELY USED** - Only for actions with NO CLI/API. Droid automates everything possible first.
...
See: references/cli-automation.md for what Droid can automate.

**Execution:** Droid automates everything with CLI/API, stops only for truly unavoidable manual steps.
</type>

<type name="checkpoint:human-verify">
**Human must verify Droid's work** - Visual checks, UX testing.
...
**Execution:** Droid builds the feature, stops, provides testing instructions, waits for approval/feedback.
</type>
...
**Execution:** Droid presents options with balanced pros/cons, waits for decision, proceeds with chosen direction.
</type>

**When to use checkpoints:**
- Visual/UX verification (after Droid builds) → `checkpoint:human-verify`
...
- Anything with CLI/API (Droid automates it) → `type="auto"`
...
**Golden rule:** If Droid CAN automate it, Droid MUST automate it. See: references/cli-automation.md
...
Reference files that Droid needs to understand before implementing.
</context_references>
...
Droid: "How? What type? What library? Where?"
</too_vague>
...
Droid can implement this immediately.
</just_right>

<too_detailed>
Writing the actual code in the plan. Trust Droid to implement from clear instructions.
</too_detailed>
</specificity_levels>
...
These require Droid to decide WHAT to do. Specify it.
</vague_actions>
...
Droid doesn't know your standards. Be explicit.
</missing_context>
</anti_patterns>

<sizing_tasks>
Good task size: 15-60 minutes of Droid work.
