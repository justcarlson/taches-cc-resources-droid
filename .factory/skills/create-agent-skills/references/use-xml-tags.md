<overview>
Skills use pure XML structure for consistent parsing, efficient token usage, and improved Droid performance. This reference defines the required and conditional XML tags for skill authoring, along with intelligence rules for tag selection.
</overview>
...
**Purpose**: Immediate, actionable guidance. Gets Droid started quickly without reading advanced sections.
...
<tag name="context">
**When to use**: Background or situational information that Droid needs before starting.
...
**When to use**: Common mistakes that Droid should avoid.
...
2. Observe where Droid struggles or succeeds
...
Markdown headings are just visual formatting. Droid must infer meaning from heading text.
</parsing_accuracy>
...
Reduce parsing errors. Droid knows exactly where sections begin and end.
</benefit>
