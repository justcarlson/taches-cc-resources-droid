---
name: create-droids
description: Expert guidance for creating, building, and using Factory CLI droids and the Task tool. Use when working with droids, setting up agent configurations, understanding how agents work, or using the Task tool to launch specialized agents.
---

<objective>
Droids are specialized Droid instances that run in isolated contexts with focused roles and limited tool access. This skill teaches you how to create effective droids, write strong system prompts, configure tool access, and orchestrate multi-agent workflows using the Task tool.

Droids enable delegation of complex tasks to specialized agents that operate autonomously without user interaction, returning their final output to the main conversation.
</objective>
...
3. Choose project-level (`.factory/agents/`) or user-level (`~/.factory/agents/`)
4. Define the droid:
   - **name**: lowercase-with-hyphens
   - **description**: When should this droid be used?
   - **tools**: Optional comma-separated list (inherits all if omitted)
   - **model**: Optional (`sonnet`, `opus`, `haiku`, or `inherit`)
5. Write the system prompt (the droid's instructions)
</workflow>
...
Project-level droids override user-level when names conflict.
</file_structure>
...
- Include when Droid should invoke this droid
- Used for automatic droid selection
</field>
...
- If omitted: defaults to configured droid model (usually sonnet)
</field>
</configuration>

<execution_model>
<critical_constraint>
**Droids are black boxes that cannot interact with users.**

Droids run in isolated contexts and return their final output to the main conversation. They:
...
- ❌ **User never sees droid's intermediate steps**

The main conversation sees only the droid's final report/output.
</critical_constraint>

<workflow_design>
**Designing workflows with droids:**
...
Main Chat: Ask user for requirements (AskUserQuestion)
  ↓
Droid: Research API and create documentation (no user interaction)
  ↓
Main Chat: Review research with user, confirm approach
  ↓
Droid: Generate code based on confirmed plan
  ↓
...
Clearly define the droid's role, capabilities, and constraints.
</principle>
...
Tailor instructions to the specific task domain. Don't create generic "helper" droids.
...
<droid_xml_structure>
Droid.md files are system prompts consumed only by Droid. Like skills and slash commands, they should use pure XML structure for optimal parsing and token efficiency.

<recommended_tags>
Common tags for droid structure:

- `<role>` - Who the droid is and what it does
...
For XML structure principles and token efficiency details, see @skills/create-agent-skills/references/use-xml-tags.md - the same principles apply to droids.
</critical_rule>
</droid_xml_structure>

<invocation>
<automatic>
Droid automatically selects droids based on the `description` field when it matches the current task.
</automatic>

<explicit>
You can explicitly invoke a droid:
...
```
> Have the test-writer droid create tests for the new API endpoints
```
</explicit>
</invocation>
...
- View all available droids
- Create new droids
- Edit existing droids
- Delete custom droids
</using_agents_command>

<manual_editing>
You can also edit droid files directly:
...
**Droid usage and configuration**: [references/droids.md](references/droids.md)
...
**Writing effective prompts**: [references/writing-droid-prompts.md](references/writing-droid-prompts.md)
